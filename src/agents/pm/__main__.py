#!/usr/bin/env python3
"""PM Agent - A2A Server Entry Point.

This agent specializes in product management, requirements analysis, and technical specifications.
...
from agents.pm.config import PM_CONFIG


def main():
    """Start the PM Agent A2A server."""
    logger = TerminalLogger("pm-agent")

    logger.section("PM Agent Starting")
    logger.info(f"Agent: {PM_CONFIG.name}")
    logger.info(f"Role: {PM_CONFIG.role}")
    logger.info(f"Port: {PM_CONFIG.port}")
    logger.info(f"URL: {PM_CONFIG.url}")
    logger.info(f"Workspace: {PM_CONFIG.workspace}")
    logger.print()

    # Create agent card
    logger.info("Creating AgentCard...")
    agent_card = create_agent_card(PM_CONFIG)
    logger.success(f"✓ AgentCard created: {agent_card.name}")

    # Create agent executor (BaseAgent)
    logger.info("Initializing BaseAgent...")
    agent_executor = BaseAgent(PM_CONFIG)
    ...
    logger.panel(
        f"PM Agent is ready!\n\n"
        f"AgentCard: {PM_CONFIG.url}/.well-known/agent.json\n"
        f"Workspace: {PM_CONFIG.workspace}\n\n"
        f"Capabilities:\n" + "\n".join(f"  • {cap}" for cap in PM_CONFIG.capabilities[:5]) + "\n  ...",
        title="Agent Ready",
        style="green"
    )
    logger.print()
    ...
    logger.info(f"Starting A2A server on {PM_CONFIG.url}")
    logger.print()
